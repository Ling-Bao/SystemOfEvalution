...
xlswrite('output.xls',outputx(1:n-1,:),1,'A2');

k1=n+2;
string_1=['A',num2str(k1)];
xlswrite('output.xls','松弛变量值',1,string_1);
string_2=['A',num2str(n+3)];
xlswrite('output.xls',outputx(n:n+m-1,:),1,string_2);

string_3=['A',num2str(n+m+4)];
xlswrite('output.xls','剩余变量值',1,string_3);
string_4=['A',num2str(n+m+6)];
xlswrite('output.xls',outputx(n+m:nA-2,:),1,string_4);

string_5=['A',num2str(n+m+s+7)];
xlswrite('output.xls','theta值',1,string_5);
string_6=['A',num2str(n+m+s+8)];
xlswrite('output.xls',outputx(nA-1,:),1,string_6);
